...
async def export_sales_data(
    ...
    try:
        ...
        # Get sales with aggregation to include customer info
        pipeline = [
            {"$match": query},
            {
                "$lookup": {
        ...
        sales = await db.sales.aggregate(pipeline).to_list(None)
        ...
        for row, sale in enumerate(sales, 2):
            ...
            worksheet.cell(row=row, column=2, value=sale.get("customer_name", "N/A"))
            worksheet.cell(row=row, column=3, value=sale.get("customer_phone", "N/A"))
