...
# Global webhook rotation state (per session)
webhook_rotation_state = {
    "current_cycle": 1,
    "current_webhook_index": 0,
    "requests_in_current_webhook": 0,
    "total_requests_processed": 0
}

async def get_active_webhooks():
    """Get list of active webhooks from database"""
    try:
        webhooks = await db.webhook_configs.find({"is_active": True}).to_list(length=100)
        if not webhooks:
            # Return default webhook if no custom webhooks configured
            return [{
                "id": "default",
                "name": "Default Webhook",
                "url": "https://n8n.phamthanh.net/webhook/checkbill",
                "is_active": True
            }]
        return [parse_from_mongo(webhook) for webhook in webhooks]
    except Exception as e:
        print(f"[ERROR] Failed to get webhooks: {e}")
...
async def get_next_webhook_url():
    """Get next webhook URL using sequential multi-cycle distribution"""
    global webhook_rotation_state
    
    active_webhooks = await get_active_webhooks()
    if not active_webhooks:
        return "https://n8n.phamthanh.net/webhook/checkbill"  # Fallback
    
    # Reset if we've completed all webhooks in current cycle
    if webhook_rotation_state["current_webhook_index"] >= len(active_webhooks):
        webhook_rotation_state["current_webhook_index"] = 0
        webhook_rotation_state["current_cycle"] += 1
        print(f"[DEBUG] Starting cycle {webhook_rotation_state['current_cycle']}")
    
    # Get current webhook
    current_webhook = active_webhooks[webhook_rotation_state["current_webhook_index"]]
    current_url = current_webhook["url"]
    
    webhook_rotation_state["requests_in_current_webhook"] += 1
    webhook_rotation_state["total_requests_processed"] += 1
    
    print(f"[DEBUG] Using webhook {webhook_rotation_state['current_webhook_index'] + 1}/{len(active_webhooks)} "
          f"(Cycle {webhook_rotation_state['current_cycle']}, "
          f"Request {webhook_rotation_state['requests_in_current_webhook']}/5)")
    
    # Move to next webhook after 5 requests
    if webhook_rotation_state["requests_in_current_webhook"] >= 5:
        webhook_rotation_state["current_webhook_index"] += 1
        webhook_rotation_state["requests_in_current_webhook"] = 0
        print(f"[DEBUG] Moving to next webhook after 5 requests")
    
    return current_url

def reset_webhook_rotation():
    """Reset webhook rotation state for new batch"""
    global webhook_rotation_state
    webhook_rotation_state = {
        "current_cycle": 1,
        "current_webhook_index": 0, 
...
@api_router.get("/admin/webhooks", response_model=List[WebhookConfig])
async def get_webhooks(current_user: dict = admin_required):
    """Get all webhook configurations (Admin only)"""
    try:
        webhooks = await db.webhook_configs.find({}).to_list(length=100)
        return [WebhookConfig(**parse_from_mongo(webhook)) for webhook in webhooks]
    except Exception as e:
        logger.error(f"Error getting webhooks: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@api_router.post("/admin/webhooks", response_model=WebhookConfig)
async def create_webhook(webhook_data: WebhookConfigCreate, current_user: dict = admin_required):
    ...
    try:
        ...
        existing = await db.webhook_configs.find_one({"url": webhook_data.url})
        if existing:
        ...
        webhook_dict["created_at"] = datetime.now(timezone.utc)
        webhook_dict["updated_at"] = datetime.now(timezone.utc)
        
        await db.webhook_configs.insert_one(webhook_dict)
        
        return WebhookConfig(**webhook_dict)
    except HTTPException:
        raise
    except Exception as e:
...
@api_router.put("/admin/webhooks/{webhook_id}", response_model=WebhookConfig)
async def update_webhook(webhook_id: str, webhook_data: WebhookConfigUpdate, current_user: dict = admin_required):
    """Update webhook configuration (Admin only)"""
    try:
        update_data = {k: v for k, v in webhook_data.dict().items() if v is not None}
        if not update_data:
        ...
        if "url" in update_data:
            ...
            existing = await db.webhook_configs.find_one({
                "url": update_data["url"],
                "id": {"$ne": webhook_id}
            })
            if existing:
        ...
        updated_webhook = await db.webhook_configs.find_one({"id": webhook_id})
        return WebhookConfig(**parse_from_mongo(updated_webhook))
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_webhook(webhook_id: str, current_user: dict = admin_required):
    """Delete webhook configuration (Admin only)"""
    try:
        result = await db.webhook_configs.delete_one({"id": webhook_id})
...
@api_router.post("/admin/webhooks/{webhook_id}/test", response_model=WebhookTestResult)
async def test_webhook(webhook_id: str, current_user: dict = admin_required):
...
@api_router.post("/admin/webhooks/test-all")
async def test_all_webhooks(current_user: dict = admin_required):
    """Test all active webhooks (Admin only)"""
    try:
        active_webhooks = await get_active_webhooks()
        results = []
        
        for webhook in active_webhooks:
            if webhook["id"] != "default":  # Skip default webhook
                result = await test_webhook(webhook["id"], current_user)
                results.append(result)
